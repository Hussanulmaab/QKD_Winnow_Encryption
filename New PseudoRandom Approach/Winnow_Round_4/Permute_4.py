...
with open("../Winnow_Round_3/Alice_Winnow-3_Final.txt", "r") as alice_file:
    alice_data = np.array([int(bit) for bit in alice_file.read().strip()])

with open("../Winnow_Round_3/Bob_Winnow-3_Final.txt", "r") as bob_file:
...
with open("Alice_Permuted_4.txt", "w") as alice_permuted_file:
    alice_permuted_file.write("".join(map(str, alice_permuted)))

with open("Bob_Permuted_4.txt", "w") as bob_permuted_file:
